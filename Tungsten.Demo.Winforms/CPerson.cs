...
namespace Tungsten.Demo.Winforms
{
    ...
    public class CPerson : PropertyHostNotifier
    {
        public Property<CPerson, string> Last { get; } = new Property<CPerson, string>(
        (vm, oldValue, newValue) =>
        {
            Log.i("Last Named = {0}", newValue);
            vm.FullName.Value = newValue + ", " + vm.First.Value;
        });
        public Property<CPerson, string> First { get; } = new Property<CPerson, string>(
        (vm, oldValue, newValue) =>
        {
            Log.i("First Named = {0}", newValue);
            vm.FullName.Value = vm.Last.Value + ", " + newValue;
        });
        public Property<CPerson, string> FullName { get; } = new Property<CPerson, string>();
        public Property<CPerson, bool> IsBusy { get; } = new Property<CPerson, bool>(false);
        public Property<CPerson, int> SaveProgress { get; } = new Property<CPerson, int>();
